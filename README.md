...
- **시뮬 + 방식A 실전(옵션)**: 같은 점검에서 조건 통과 중 **신호점수 순**으로 실주문(기본 **세션 최대 3회**, 동시 보유는 총한도로 ≈2종), 나머지는 **병렬 시뮬** / 익절 +5% · 손절 −2% (보유 청산은 기본 60초마다)
- **워치 스킵 요약**: 미진입 사유를 주기적으로 텔레그램 알림
- **중간·마감 보고**: NY 12:00 이후 중간 보고(세션 중 재시도), NY 정규장 종료 시 최종 손익 보고(놓치면 주말에도 재시도)
- **동적·유동 워치**: 급증/상승/거래량 → (비면) 거래대금/체결강도/신고가 → 점수 통과분만 최대 10종 (고정 10개 채움 아님). **ETP 제외·일반주만**
- `/health` HTTP (Railway)
