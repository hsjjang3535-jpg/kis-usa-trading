...
| `US_SIM_TAKE_PROFIT_PCT` | `6.0` | 익절 % (하드). 왕복수수료~0.5% 반영 기본 |
| `US_FEE_ONE_WAY_PCT` | `0.25` | 매수·매도 각 수수료 %. 알림 순손익·손익비 참고 |
...
- 매수 시 원/달러 환율을 포지션에 기록하고, 매도 알림에서 비교합니다.
- 매도환율이 매수 때보다 **높으면** `환전 검토 가능`, **낮으면** `달러 보유 권장`을 안내합니다.
- **환전 주문은 봇이 넣지 않습니다.** 원할 때만 한투 앱에서 수동 환전하세요.
- 한투 앱 **자동환전**이 켜져 있으면 끄세요.
- `US_SIM_TAKE_PROFIT_PCT` 기본 `6.0`은 편도 `0.25%`×2 왕복을 감안한 값입니다. Railway에 `5.0`이 남아 있으면 삭제하거나 `6.0`으로 바꾸세요.
